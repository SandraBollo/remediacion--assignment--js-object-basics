...
function makeFullNamesList(obj) {
  ...
  let arraynw = obj[i]
    ...
    if (arraynw.gender === 'female') {
       let femaleFull = 'Ms.' + ' ' + arraynw.first + ' ' + arraynw.last
         newObj.push(femaleFull)

         console.log(femaleFull);

    }else if (arraynw.gender === 'male') {
      let maleFull = 'Mr.' + ' ' + arraynw.first + ' ' + arraynw.last

      newObj.push(maleFull);
         console.log(maleFull);
    }

  }
  console.log("resultado", newObj);
 return newObj
}
